...
public class TypeServiceChosessHome extends RecyclerView.Adapter<TypeServiceChosessHome.viewHolder>{
    private Context mContext;
    public List<TypeService.ItemsEntity> typeServiceSelectedList;
    private int onClick = -1;

    public TypeServiceChosessHome(Context context){
    ...
    @NonNull
    @Override
    public TypeServiceChosessHome.viewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.item_service_chosess , parent,false);
        return new TypeServiceChosessHome.viewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull TypeServiceChosessHome.viewHolder holder, @SuppressLint("RecyclerView") int position) {
